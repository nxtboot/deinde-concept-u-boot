						    ...
						    void *data)
{
	...
	case EFI_IP4_CONFIG2_DATA_TYPE_MANUAL_ADDRESS:
		if (net->ip4_policy != EFI_IP4_CONFIG2_POLICY_STATIC)
			return EFI_EXIT(EFI_WRITE_PROTECTED);
		if (!data_size && !data) {
			memset((void *)&net->ip4_addr, 0,
			       sizeof(net->ip4_addr));
			return EFI_EXIT(EFI_SUCCESS);
		}
		if (data && data_size == sizeof(struct efi_ip4_config2_manual_address)) {
			memcpy((void *)&net->ip4_addr, data,
			       sizeof(struct efi_ip4_config2_manual_address));
			efi_net_set_addr(&net->ip4_addr.address,
					 &net->ip4_addr.subnet_mask, NULL, NULL);
			return EFI_EXIT(EFI_SUCCESS);
		}
	...
	case EFI_IP4_CONFIG2_DATA_TYPE_POLICY:
		if (data && data_size == sizeof(enum efi_ip4_config2_policy)) {
			net->ip4_policy = *(enum efi_ip4_config2_policy *)data;
			return EFI_EXIT(EFI_SUCCESS);
		}
		return EFI_EXIT(EFI_BAD_BUFFER_SIZE);
						    ...
						    void *data)
{
	...
	case EFI_IP4_CONFIG2_DATA_TYPE_INTERFACEINFO:
		...
		memcpy(info->hw_address.mac_addr, net->mac_addr, 6);
	...
	case EFI_IP4_CONFIG2_DATA_TYPE_MANUAL_ADDRESS:
		...
		efi_net_get_addr(&net->ip4_addr.address,
				 &net->ip4_addr.subnet_mask, NULL, NULL);
		memcpy(data, (void *)&net->ip4_addr,
		       sizeof(struct efi_ip4_config2_manual_address));
				   ...
				   struct efi_ip4_config2_protocol *ip4config)
{
	...
	memcpy(net->mac_addr, eth_get_ethaddr(), 6);
