...
static const char *reqname(unsigned int r)
{
	switch (r) {
...
static void mv_ep_submit_next_request(struct mv_ep *mv_ep)
{
	...
	head->next = (unsigned int)(ulong)item;
	head->info = 0;
...
{
	struct mv_udc *udc = (struct mv_udc *)controller.ctrl->hccr;
	unsigned int n = readl(&udc->usbsts);
...
static int mv_pullup(struct usb_gadget *gadget, int is_on)
{
	...
	if (is_on) {
		...
		writel((unsigned int)(ulong)controller.epts, &udc->epinitaddr);
