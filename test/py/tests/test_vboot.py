...
def test_vboot_ext_data_bounds(ubman):
    ...
    def run_bootm(test_type, expect_string):
        ...
        Args:
            ...
            expect_string: A string which is expected in the output.
        """
        # Load the FIT at the start of RAM so that the largest in-bounds
        # data region (offset 0, size FIT_SIGNATURE_MAX_SIZE) ends exactly
        # at the end of sandbox RAM, avoiding an out-of-bounds read when
        # the hash is calculated
        ubman.restart_uboot()
        with ubman.log.section('Verified boot %s %s' % (sha_algo, test_type)):
            output = ubman.run_command_list(
                ['host load hostfs - 0 %s' % fit,
                 'fdt addr 0',
                 'bootm 0'])
        assert expect_string in ''.join(output)
